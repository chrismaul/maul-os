...
mkdir -p $OUTPUT
pacman --needed -Sy --noconfirm arch-install-scripts base-devel git sudo rsync
...
sed -i 's/^HOOKS=.*$/HOOKS=(base systemd keyboard sd-vconsole sd-vroot modconf sd-lvm2 sd-encrypt block filesystems fsck)/' $OUTPUT/etc/mkinitcpio.conf
sed -i 's/^MODULES=.*$/MODULES=(squashfs virtio virtio_pci virtio_blk ext4)/' $OUTPUT/etc/mkinitcpio.conf
