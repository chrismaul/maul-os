...
then
  echo "git tags"
  git tag
  LATEST_VERISON=$(git tag| grep -o "v[0-9][0-9]*.[0-9][0-9]*.[0-9][0-9]*" | sort -Vr | head -n1)
  ...
  CURRENT_VERISON=$(git tag -l --points-at HEAD| grep -o "v[0-9][0-9]*.[0-9][0-9]*.[0-9][0-9]*" | sort -Vr | head -n1)
